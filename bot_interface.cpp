...
void GameState::ParseData(string toParse)
{
	...
	while(toParse.size() > 0)
	{
		...
		else if(type.compare("ship") == 0)
		{
			...
			{
				ships[uid]->uid = uid;
				ships[uid]->pos << posx, posy;
				ships[uid]->vel << velx, vely;
		...
		else if(type.compare("rock") == 0)
		{
			...
			{
				rocks[uid]->uid = uid;
				rocks[uid]->pos << posx, posy;
				rocks[uid]->vel << velx, vely;
		...
		else if(type.compare("laser") == 0)
		{
			...
			{
				lasers[uid]->uid = uid;
				lasers[uid]->pos << posx, posy;
				lasers[uid]->vel << velx, vely;
	...
	for(map<int, Ship*>::iterator itr = ships.begin(); itr != ships.end();)
    {
        ...
        for(unsigned i = 0; i < shipIds.size(); ++i)
        {
    ...
    for(map<int, Rock*>::iterator itr = rocks.begin(); itr != rocks.end();)
    {
       bool found = false;
        ...
        for(unsigned i = 0; i < rockIds.size(); ++i)
        {
    ...
    for(map<int, Laser*>::iterator itr = lasers.begin(); itr != lasers.end();)
    {
       bool found = false;
        ...
        for(unsigned i = 0; i < laserIds.size(); ++i)
        {
